/**
 * BUYERO FIREBASE CLOUD MESSAGING SERVICE WORKER (Admin scoped)
 */
...
messaging.onBackgroundMessage((payload) => {
  console.log('[Buyero Admin SW] Background Push:', payload);

  const notificationTitle = payload.notification?.title || payload.data?.title || 'Buyero Admin Alert';
  const notificationBody = payload.notification?.body || payload.data?.body || 'New administrative event received';
  ...
  const targetUrl = payload.fcmOptions?.link || payload.data?.url || payload.data?.link || '/admin/index.html';
...
self.addEventListener('notificationclick', (event) => {
  ...
  const destinationUrl = event.notification.data?.url || '/admin/index.html';
  event.waitUntil(
